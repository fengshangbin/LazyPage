...
$(function() {
  autoEvent();
  file = getfile();
  if (parent && parent.getRealDate != undefined) {
    var nowDate = parent.getRealDate();
    dayWeek = getServerWeek(nowDate);
  } else {
    dayWeek = getDayWeek();
  }
  getList();
...
function checkWeekDay() {
  ...
  if (parent && parent.getRealDate != undefined) {
    var nowDate = parent.getRealDate();
    now = nowServerTime(nowDate);
    now = changeStrToMinutes(now);
  } else {
    now = changeStrToMinutes(nowTime());
  }
  console.log(now);
  var next = 24 * 60;
  setTimeout(function() {
    ...
    nextPlayTime = 1;
    $('.test').html('change day video list');
    getList();
  }, (next - now) * 60 * 1000);
}
...
function setSeekTime() {
  ...
  set = setTimeout(function() {
    vEventList();
    $('.test').html('loop list video');
  }, leaveTime);

  if (nowVideo == 'videoPlay') {
    if (nextPlayTime == -1 || nextPlayTime == 1) {
      $('.videoWarp').append(v);
      if (seek > 20) {
